...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "TDCNetwork") type = TDCNetwork;
    if(strType == "TDCNetworkVariable") type = TDCNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == TDCNetwork) s = "TDCNetwork";
    if(type == TDCNetworkVariable) s = "TDCNetworkVariable";
