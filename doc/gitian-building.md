...
*Setup instructions for a Gitian build of TDC Core using a Debian VM or physical system.*
...
- [Building TDC Core](#building-TDC-core)
...
Clone the git repositories for TDC Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/TDCpay/TDC
```
...
in the TDC Core repository under 'Fetch and build inputs' to install sources which require
...
To build TDC Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the TDC Core repository.
...
```bash
    ...
    From https://github.com/TDCpay/TDC
...
For example:
```bash
URL=https://github.com/crowning-/TDC.git
...
and the TDC git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../TDC/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/TDC.git
SIGPATH=/some/root/path/TDC-detached-sigs.git

./bin/gbuild --url TDC=${BTCPATH},signature=${SIGPATH} ../TDC/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/TDC-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/TDC-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/TDC-osx-build.assert
```
...
[TDC/gitian.sigs](https://github.com/TDCpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
